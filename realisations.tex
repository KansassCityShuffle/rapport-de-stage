...
Parallèlement, $M_{4}$ communique directement des ordres de vitesse et de direction au Wifibot par le biais de l'interface WiFi $i_{3}$. 
...
de ce type. \textbf{\`{A} titre d'exemple, le fichier \path{sensor_msgs/LaserScan.msg} est présenté en annexe X.}
...
Ces différents composants et la manière dont ils ont été réalisés font l'objet de la partie \ref{subsection:ROSnodes}. 
...
\begin{figure}[h]
  \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{figures/all_nodes}}%
...
Les éléments contenus par le header sont \path{tID}, un identifiant donnant le type de la donnée transmise, \path{pSize}, la taille du payload et un \path{padding} à $0$ qui vise à ce qu'il atteigne 20 octets. 
...
D'un point de vue formel, le fonctionnement de l'application est régi par une machine à état relativement simple, représentée figure \ref{fig:stateMachine}.
...
Qt offre un système de paramétrage des applications par le biais de fichiers texte et d'une classe \path{QSettings} indépendante de la plateforme utilisée
...
\path{NodesKiller}. 
Ces objets exécutent d'un shell Linux (\path{/bin/bash}) au sein d'un membre Qt de type \path{QProcess} permettant l'éxecution du launchfile afférent lorsque l'utilisateur presse le bouton ``Start'' du module.
Ils sont aussi responsables de la redirection des sorties du processus vers les sorties dédiés à la journalisation des évènements de l'application\footnote{La classe \path{LogsHandler} est utilisée pour journaliser les messages d'une part dans des fichiers datés du répertoire d'exécution 
